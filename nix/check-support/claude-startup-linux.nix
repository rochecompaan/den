...
pkgs.runCommand "claude-startup"
  ...
  ''
    ...
    expect_rejected() {
      rm -f "$rootHost/fence.marker" "$rootHost/agent.log" "$rootHost/rejected-policy.json" "$rootHost/rejected.out" "$rootHost/rejected.err"
      ...
      test ! -e "$rootHost/rejected-policy.json"
