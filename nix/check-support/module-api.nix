...
let
  ...
  homeDisabledOutPaths = packageOutPaths homeDisabled.config.home.packages;
  devenvDisabledOutPaths = packageOutPaths devenvDisabled.packages;
  homeDefaultOutPaths = packageOutPaths homeDefaultPackages;
  devenvDefaultOutPaths = packageOutPaths devenvDefaultPackages;
  withoutOutPath = outPath: paths: builtins.filter (path: path != outPath) paths;
...
assert builtins.length homeDefaultOutPaths == builtins.length homeDisabledOutPaths + 1;
assert sorted (withoutOutPath expectedDefault.outPath homeDefaultOutPaths) == sorted homeDisabledOutPaths;
assert countOutPath expectedDefault.outPath devenvDefaultPackages == 1;
assert builtins.length devenvDefaultOutPaths == builtins.length devenvDisabledOutPaths + 1;
assert sorted (withoutOutPath expectedDefault.outPath devenvDefaultOutPaths) == sorted devenvDisabledOutPaths;
