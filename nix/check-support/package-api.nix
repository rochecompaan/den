...
let
  ...
  normalize = import ../lib/options.nix { inherit pkgs; };
  adapter = (import ../lib/mk-claude.nix {
...
assert !(den.packages ? resources);
assert !(den.lib ? mkAgentSandbox);
assert !(den.lib ? mkResources);
...
assert fails (normalize { resourceBundles = [ ]; });
assert fails (normalize { claudeResources = [ ]; });
pkgs.runCommand "package-api"
  ...
  ''
    ...
    grep -Fqx ${fakeGh} "$closure"
    grep -Fqx ${fakeDocker} "$closure"
    grep -Fqx ${fakePodman} "$closure"
    touch "$out"
  ''
