...
let
  ...
  patched = upstream.overrideAttrs (old: {
    patches = (old.patches or [ ]) ++ [ patch readOnlyMaskPatch bootstrapAttestationPatch ];
    checkPhase = ''
      runHook preCheck
...
{
  package = patched;
  inherit capabilities patchHash readOnlyMaskPatchHash bootstrapAttestationPatchHash;
